...
@section('content')
                            ...
                            <table class="table table-hover mb-2 table-bordered">
                                ...
                                <tbody data-document-products>
                                    ...
                                    <tr class="text-right">
                                        ...
                                        @if(!userIsWarehouse())
                                        <td>{{ format_price($document->useMpcPrice() ? $item->mpc : $item->vpc) }}</td>
                                        <td>{{ format_price($item->fiscal_net_price - $item->fiscal_net_discounted_price) }}</td>
                                        ...
                                        <td>{{ format_price($item->fiscal_discounted_price) }}</td>
                                            @if(true)
                                        <td><strong>{{ format_price($item->qty * $item->fiscal_discounted_price, 2) }}</strong></td>
                                            @else
                    ...
                    <div class="row">
                                ...
                                <table class="table">
                                    <tbody>@php $price_for_discount = $document->useMpcPrice() ? $document->total : $document->subtotal; @endphp
                                        <tr>@php $discount_value1 = getDiscountedValue($document->discount1, $price_for_discount); $discounted = ($discount_value1 > 0); @endphp
                                            ...
                                            <td class="text-right @if($discounted){{ 'text-success' }}@endif">@if($discounted){{ '- ' }}@endif{{ format_price($discount_value1, 2) }} {{ $document->currency }}</td>
                                        </tr>
                                        <tr>@php $discount_value2 = getDiscountedValue($document->discount2, $price_for_discount - $discount_value1); $discounted = ($discount_value2 > 0); @endphp
                                            ...
                                            <td class="text-right @if($discounted){{ 'text-success' }}@endif">@if($discounted){{ '- ' }}@endif{{ format_price($discount_value2, 2) }} {{ $document->currency }}</td>
                                        </tr>
                                        <tr>@php $discount_value3 = getDiscountedValue($document->discount3, $price_for_discount - $discount_value1 - $discount_value2); $discounted = ($discount_value3 > 0); @endphp
                                            <th class="text-uppercase">{{ trans('document.data.discount_num', ['num' => 3, 'discount' => $document->discount3]) }}</th>
                                            <td class="text-right @if($discounted){{ 'text-success' }}@endif">@if($discounted){{ '- ' }}@endif{{ format_price($discount_value3, 2) }} {{ $document->currency }}</td>
                                        </tr>
                                        <tr>@php $discount_total = $discount_value1 + $discount_value2 + $discount_value3; @endphp
                                            <th class="text-uppercase">{{ trans('document.data.total_discount') }}</th>
                                            <td class="text-right @if($discount_total){{ 'text-success' }}@endif"><strong>@if($discount_total){{ '- ' }}@endif{{ format_price($discount_total, 2) }} {{ $document->currency }}</strong></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="col-12 col-md-6 mb-2">
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        <tr>@php $price = $document->useMpcPrice() ? getPriceWithoutVat($document->total_discounted, $document->tax_rate) : $document->subtotal_discounted;// getPriceWithoutVat($net_tax, $document->tax_rate); @endphp
                                        ...
                                        <tr>@php $price_with_vat = round($price, 2) + round($vat, 2); @endphp
    ...
    <!-- end: content body -->
    @if((config('app.env') == 'local') || (config('app.url') == 'https://dev.enabavka.ba') || (request('fiscal_debug') == '1'))
        <div class="card">
            <div class="card-body">
                <div class="table-responsive">
